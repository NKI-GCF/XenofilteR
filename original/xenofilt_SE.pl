...
#===============================================================================
#
# Loop over primary host (mouse) alignments. Store the number of mismatches plus the
# number of clipped NTs multiplied by 1000 / read_quality. If the read is not mapped
# instead assign the exceeding balance of readlength * 1000. The
# readname + first/second [read in pair] are used as key.
#
...
HOSTLINE: while (<HOST>) {
	...
	if (($e[4] == 0) || ($e[5] eq '*')) { # entire fail is 1000 * readlength
		...
		if ($i != 10) {
			...
			$h{$e[0]} = $i * 1000 / $e[4]; # matches * 1000 / read_quality
		} else { # entire fail
			$h{$e[0]} = length($e[9]) * 1000;
		}
	}
...
while ($L) {
	...
	if (exists $h{$1}) {
		...
		die if exists $single{$id};
		my $balance = $h{$id};
		if ($balance > 0) { # both say it's more likely human
