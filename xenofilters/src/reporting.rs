...
impl<'a> RunReport<'a> {
    pub(crate) fn print_routing_counters(&self) {
        ...
        for nr in 0..stream_count {
            ...
            tracing::info!(
                stream = nr, backend = self.backend,
                discard = self.counters[b], out = self.counters[b + 1],
                ambiguous = self.counters[b + 2], chimeric = self.counters[b + 3],
                "Stream summary"
            );
        }
        let total: u64 = self.counters.iter().sum();
        if total == 0 { return; }
        let ambiguous: u64 = self.counters.chunks_exact(COUNTER_STRIDE).map(|c| c[2]).sum();
