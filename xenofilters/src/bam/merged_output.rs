//! `src/bam/merged_output.rs`
//!
//! Single-file merged BAM output with read-group postfix routing.
//!
//! # Design
//!
...
pub(crate) fn expand_header(mut header: Header) -> Header {
    ...
    let additions: Vec<(String, Map<ReadGroup>)> = header
        ...
        .flat_map(|(id, rg)| {
            ...
            [SUFFIX_FILTERED, SUFFIX_AMBIGUOUS].map(|suffix| {
                let new_id = format!("{id_str}{suffix}");
                (new_id, rg.clone())
            })
        })
        .collect();
...
mod tests {
    ...
    fn expand_header_adds_two_entries_per_rg() {
        ...
        let expanded = expand_header(h);
        let ids = rg_ids(&expanded);
        // 2 original + 4 derived = 6
        assert_eq!(ids.len(), 6);
        assert!(ids.contains(&"rg0".to_string()));
        assert!(ids.contains(&format!("rg0{SUFFIX_FILTERED}")));
        assert!(ids.contains(&format!("rg0{SUFFIX_AMBIGUOUS}")));
        assert!(ids.contains(&"rg1".to_string()));
        assert!(ids.contains(&format!("rg1{SUFFIX_FILTERED}")));
        assert!(ids.contains(&format!("rg1{SUFFIX_AMBIGUOUS}")));
    }
