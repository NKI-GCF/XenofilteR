...
pub(crate) fn parse_variant_core(record: &RecordBuf, header: &Header) -> Result<VariantCore, Error> {
    let chrom = record.reference_sequence_name().to_string();
    let ref_id = header.contigs().get_index_of(&chrom).ok_or(Error::NoReferenceSequenceId)?;
