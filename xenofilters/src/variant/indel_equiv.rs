...
fn classify(ref_a: &[u8], alt_a: &[u8]) -> IndelKind {
...
/// the reference.  Typically `std::io::BufReader<std::fs::File>`.
pub(crate) struct IndelEquivalenceExpander<R: BufRead + Seek> {
    fasta: fasta::IndexedReader<R>,
}

impl<R: BufRead + Seek> IndelEquivalenceExpander<R> {
    pub(crate) fn new(fasta: fasta::IndexedReader<R>) -> Self {
        Self { fasta }
    }
    ...
    fn fetch_context(
        ...
        // noodles positions are 1-based and inclusive.
        let start_1based =
            Position::new(ctx_start + 1).context("position overflow in FASTA fetch")?;
        let end_1based = Position::new(ctx_end).context("position overflow in FASTA fetch")?;

        let region = format!("{chrom}:{}-{}", ctx_start + 1, ctx_end);
        let record = self
            .fasta
            .query(&region.parse().context("invalid region string")?)
            .with_context(|| format!("FASTA query failed for {region}"))?;
    ...
    pub(crate) fn expand_sample(
        ...
        chrom: &str,
        gamete: bool,
    ) -> Result<Vec<SampleVariant>> {
        // Parse the canonical variant first (existing function; unchanged).
        let canonicals = match crate::variant::sample::parse_sample_record(record, header) {
            Ok(v) => v,
            Err(e) => {
        ...
        for canon in &canonicals {
            ...
            let (ctx_start, ctx) = self
                ...
                .with_context(|| format!("fetching context at {chrom}:{}", canon.pos() + 1))?;
    ...
    pub(crate) fn expand_population(
        ...
        let canonicals = match crate::variant::population::parse_population_record(record, header) {
            Ok(v) => v,
            Err(e) => {
...
// ---------------------------------------------------------------------------
// Store<V>::insert_expanded  (extension; keep existing internals unchanged)
// ---------------------------------------------------------------------------
//
// `Store<V>` already maintains a per-chromosome sorted Vec<V> for binary-
// search range queries.  `insert_expanded` simply calls the existing per-
// variant insert path for each element in the expanded Vec.  The hot-path
// `overlapping_multi` implementation is untouched.

use crate::variant::store::Store;
use crate::variant::Variant;

impl<V: Variant + Clone + Ord> Store<V> {
    /// Insert a batch of pre-expanded equivalent variants.
    ///
    /// Equivalent to calling `insert` once per entry; re-sorts after
    /// the batch to maintain the binary-search invariant.
    ///
    /// Called once per VCF record at startup; not on the hot path.
...
pub(crate) fn build_sample_store_expanded(
    ...
    header: &vcf::Header,
    gamete: bool,
) -> Result<Store<SampleVariant>> {
    use noodles::{bgzf, vcf::io::Reader as VcfReader};
    use std::{fs::File, io::BufReader};

    let fasta_reader = fasta::IndexedReader::new(BufReader::new(
    ...
    let mut reader: Box<dyn Iterator<Item = Result<vcf::variant::RecordBuf>>> = if is_bcf {
        use noodles::bcf;
        let mut bcf_reader =
            bcf::io::Reader::new(bgzf::io::Reader::new(BufReader::new(File::open(vcf_path)?)));
        let _hdr = bcf_reader.read_header()?;
        Box::new(
            bcf_reader
                .record_bufs(header)
        ...
        let _hdr = vcf_reader.read_header()?;
        Box::new(
            vcf_reader
                .record_bufs(header)
                .map(|r| r.map_err(anyhow::Error::from)),
    ...
    for rec_result in reader {
        let mut rec = rec_result.context("reading VCF record")?;
        // Derive chromosome name for FASTA fetch.
        let chrom_id = rec.reference_sequence_name().to_string();
        let variants = expander
            .expand_sample(&mut rec, header, &chrom_id, gamete)
            .with_context(|| format!("expanding record at {chrom_id}"))?;
        n_expanded += variants.len() as u64;
        n_canonical += 1;
        store.insert_expanded(variants);
    }
...
pub(crate) fn build_population_store_expanded(
    ...
    fasta_path: &Path,
    header: &vcf::Header,
) -> Result<Store<Population>> {
    use noodles::{bgzf, vcf::io::Reader as VcfReader};
    use std::{fs::File, io::BufReader};

    let fasta_reader = fasta::IndexedReader::new(std::io::BufReader::new(
    ...
    let mut reader: Box<dyn Iterator<Item = Result<vcf::variant::RecordBuf>>> = if is_bcf {
        use noodles::bcf;
        let mut bcf_reader = bcf::io::Reader::new(bgzf::io::Reader::new(std::io::BufReader::new(
            std::fs::File::open(vcf_path)?,
        )));
        let _hdr = bcf_reader.read_header()?;
        Box::new(
            bcf_reader
        ...
        let mut vcf_reader =
            vcf::io::Reader::new(std::io::BufReader::new(std::fs::File::open(vcf_path)?));
        let _hdr = vcf_reader.read_header()?;
        Box::new(
            vcf_reader
                .record_bufs(header)
    ...
    for rec_result in reader {
        ...
        let chrom_id = rec.reference_sequence_name().to_string();
        let variants = expander.expand_population(&mut rec, header, &chrom_id)?;
        n_expanded += variants.len() as u64;
        n_canonical += 1;
        store.insert_expanded(variants);
    }
