...
impl<R: BufRead + Seek> IndelEquivalenceExpander<R> {
    ...
    pub(crate) fn expand_sample_with_refid(
        ...
        gamete: bool,
    ) -> Result<Option<(usize, Vec<Sample>)>> {
    ...
    pub(crate) fn expand_population_with_refid(
        ...
        header: &vcf::Header,
        name_to_id: &HashMap<String, usize>,
    ) -> Result<Option<(usize, Vec<Population>)>> {
        let chrom = rec.reference_sequence_name().to_string();
        let ref_id = match name_to_id.get(&chrom) {
            Some(&id) => id,
            None => {
...
fn read_vcf_or_bcf_header(path: &Path) -> Result<vcf::Header> {
    use std::{fs::File, io::BufReader};
...
fn for_each_vcf_record<F>(path: &Path, header: &vcf::Header, mut f: F) -> Result<()>
where
    F: FnMut(&mut vcf::variant::RecordBuf) -> Result<()>,
{
    use std::{fs::File, io::BufReader};
    if path.extension().map_or(false, |e| e == "bcf") {
        let mut r = bcf::io::Reader::new(bgzf::io::Reader::new(BufReader::new(File::open(path)?)));
