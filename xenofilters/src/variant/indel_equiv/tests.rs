    ...
    fn store_insert_expanded_count() {
        ...
        let del_expanded: Vec<Population> =
            enumerate_equivalents(0, &del.ref_a, &del.alt_a, reference, 0)
                .into_iter()
                .map(|eq| del.with_alleles(eq.pos, &eq.ref_a, &eq.alt_a))
                .collect();
        assert_eq!(del_expanded.len(), 4);

        let snp_expanded: Vec<Population> =
            enumerate_equivalents(10, &snp.ref_a, &snp.alt_a, reference, 0)
                .into_iter()
                .map(|eq| snp.with_alleles(eq.pos, &eq.ref_a, &eq.alt_a))
                .collect();
