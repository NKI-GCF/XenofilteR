...
fn test_scoring_validate_ok() {
    let mut s = ScoringArgs { gap_open: 6.0, gap_extend: 1.0, mismatch_penalty: 4.0, ..Default::default() };
    assert!(s.validate().is_ok());
}

#[test]
fn test_scoring_validate_rejects_nonpositive_mismatch_penalty() {
    let mut s = ScoringArgs { gap_open: 6.0, gap_extend: 1.0, mismatch_penalty: 0.0, ..Default::default() };
...
fn test_scoring_validate_rejects_zero_gap_open() {
    let mut s = ScoringArgs { gap_open: 0.0, gap_extend: 1.0, mismatch_penalty: 4.0, ..Default::default() };
    assert!(s.validate().is_err());
}

#[test]
fn test_scoring_validate_rejects_invalid_warn_ambig_fraction() {
    let mut s = ScoringArgs { gap_open: 6.0, gap_extend: 1.0, mismatch_penalty: 4.0, warn_ambig_fraction: 1.5, ..Default::default() };
    assert!(s.validate().is_err());
}

#[test]
fn test_to_penalty_q0_is_certain_error() {
    let s = ScoringArgs { gap_open: 6.0, gap_extend: 1.0, mismatch_penalty: 4.0, ..Default::default() };
    let p = s.to_penalty();
...
fn test_to_penalty_mismatch_scaling_factor() {
    let mut s = ScoringArgs { gap_open: 6.0, gap_extend: 1.0, mismatch_penalty: 8.0, ..Default::default() };
...
fn test_io_validate_rejects_invalid_stream_count() {
    let io = IoArgs { alignment: vec!["a.bam".into()], ..Default::default() };
    assert!(io.validate(1, 2..=2).is_err());
}
...
fn test_strain_stream_count() {
    let mut args = StrainArgs {
        io: IoArgs { alignment: vec!["a.bam".into(), "b.bam".into()], ..Default::default() },
...
fn test_strain_requires_variant_profiles_for_both_strains() {
    let mut args = StrainArgs {
        io: IoArgs { alignment: vec!["single_strain.bam".into()], ..Default::default() },
