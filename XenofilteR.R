...
Human_list<-system("ls", intern=T)
Human_list<-Human_list[grep(".X1_hg19_sorted.bam", Human_list)]
Human_list<-Human_list[-grep(".bai", Human_list)]

Mouse_list<-system("ls", intern=T)
Mouse_list<-Mouse_list[grep("X1_mm10_sorted.bam", Mouse_list)]
Mouse_list<-Mouse_list[-grep(".bai", Mouse_list)]

Sample_list<-cbind(Human_list, Mouse_list)

#Sample_list<-read.table(file="/srv/nfs4/void-bulk/NKI/o.krijgsman/PDX/PDX/BAM_files_2/PDX_BAM_hg19_mm10.txt", header=T)


######## Filtering based on mapping of reads to human and mouse
